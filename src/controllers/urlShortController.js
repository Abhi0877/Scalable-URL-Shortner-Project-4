...
let urlRedrict = async function (req, res) {
    try {
        ...
        if(longUrlLink) {
            return res.status(302).redirect(longUrlLink)
        } else {
          ...
          await SET_ASYNC(`${urlCode}`, JSON.stringify(longUrlLink.longUrl),{EX: 1000, NX: true })
          return res.status(302).redirect( longUrlLink.longUrl);
